...
class GaggiMateClient {
  public:
    ...
    using SystemInfoCallback = std::function<void(const char *hardware, const char *version, uint32_t protocolVersion,
                                                  bool dimming, bool pressure, bool ledControl, bool tof, bool dualBoiler)>;
    using SensorCallback =
        std::function<void(float temperature, float temperture2, float pressure, float puckFlow, float pumpFlow, float puckResistance)>;
