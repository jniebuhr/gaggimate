#include "styles.h"
#include "images.h"
#include "fonts.h"

#include "ui.h"
#include "screens.h"

//
// Style: Container_connectivity
//

void init_style_container_connectivity_ITEMS_DEFAULT(lv_style_t *style) {
    lv_style_set_flex_flow(style, LV_FLEX_FLOW_ROW);
    lv_style_set_layout(style, LV_LAYOUT_FLEX);
    lv_style_set_flex_main_place(style, LV_FLEX_ALIGN_CENTER);
    lv_style_set_flex_cross_place(style, LV_FLEX_ALIGN_CENTER);
    lv_style_set_flex_track_place(style, LV_FLEX_ALIGN_CENTER);
    lv_style_set_translate_y(style, -210);
};

lv_style_t *get_style_container_connectivity_ITEMS_DEFAULT() {
    static lv_style_t *style;
    if (!style) {
        style = lv_mem_alloc(sizeof(lv_style_t));
        lv_style_init(style);
        init_style_container_connectivity_ITEMS_DEFAULT(style);
    }
    return style;
};

void add_style_container_connectivity(lv_obj_t *obj) {
    (void)obj;
    lv_obj_add_style(obj, get_style_container_connectivity_ITEMS_DEFAULT(), LV_PART_ITEMS | LV_STATE_DEFAULT);
};

void remove_style_container_connectivity(lv_obj_t *obj) {
    (void)obj;
    lv_obj_remove_style(obj, get_style_container_connectivity_ITEMS_DEFAULT(), LV_PART_ITEMS | LV_STATE_DEFAULT);
};

//
// Style: images-theme-color
//

void init_style_images_theme_color_MAIN_DEFAULT(lv_style_t *style) {
    lv_style_set_img_recolor(style, lv_color_hex(theme_colors[active_theme_index][0]));
    lv_style_set_img_recolor_opa(style, 255);
};

lv_style_t *get_style_images_theme_color_MAIN_DEFAULT() {
    static lv_style_t *style;
    if (!style) {
        style = lv_mem_alloc(sizeof(lv_style_t));
        lv_style_init(style);
        init_style_images_theme_color_MAIN_DEFAULT(style);
    }
    return style;
};

void add_style_images_theme_color(lv_obj_t *obj) {
    (void)obj;
    lv_obj_add_style(obj, get_style_images_theme_color_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};

void remove_style_images_theme_color(lv_obj_t *obj) {
    (void)obj;
    lv_obj_remove_style(obj, get_style_images_theme_color_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};

//
// Style: connectivity-container-image
//

void init_style_connectivity_container_image_MAIN_DEFAULT(lv_style_t *style) {
    init_style_images_theme_color_MAIN_DEFAULT(style);
    
    lv_style_set_border_opa(style, 255);
    lv_style_set_border_color(style, lv_color_hex(theme_colors[active_theme_index][1]));
    lv_style_set_border_width(style, 10);
};

lv_style_t *get_style_connectivity_container_image_MAIN_DEFAULT() {
    static lv_style_t *style;
    if (!style) {
        style = lv_mem_alloc(sizeof(lv_style_t));
        lv_style_init(style);
        init_style_connectivity_container_image_MAIN_DEFAULT(style);
    }
    return style;
};

void add_style_connectivity_container_image(lv_obj_t *obj) {
    (void)obj;
    lv_obj_add_style(obj, get_style_connectivity_container_image_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};

void remove_style_connectivity_container_image(lv_obj_t *obj) {
    (void)obj;
    lv_obj_remove_style(obj, get_style_connectivity_container_image_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};

//
// Style: Text-theme-color
//

void init_style_text_theme_color_MAIN_DEFAULT(lv_style_t *style) {
    lv_style_set_text_color(style, lv_color_hex(theme_colors[active_theme_index][0]));
    lv_style_set_text_opa(style, 255);
};

lv_style_t *get_style_text_theme_color_MAIN_DEFAULT() {
    static lv_style_t *style;
    if (!style) {
        style = lv_mem_alloc(sizeof(lv_style_t));
        lv_style_init(style);
        init_style_text_theme_color_MAIN_DEFAULT(style);
    }
    return style;
};

void add_style_text_theme_color(lv_obj_t *obj) {
    (void)obj;
    lv_obj_add_style(obj, get_style_text_theme_color_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};

void remove_style_text_theme_color(lv_obj_t *obj) {
    (void)obj;
    lv_obj_remove_style(obj, get_style_text_theme_color_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};

//
// Style: screen-theme-color
//

void init_style_screen_theme_color_MAIN_DEFAULT(lv_style_t *style) {
    lv_style_set_bg_color(style, lv_color_hex(theme_colors[active_theme_index][1]));
    lv_style_set_bg_opa(style, 255);
};

lv_style_t *get_style_screen_theme_color_MAIN_DEFAULT() {
    static lv_style_t *style;
    if (!style) {
        style = lv_mem_alloc(sizeof(lv_style_t));
        lv_style_init(style);
        init_style_screen_theme_color_MAIN_DEFAULT(style);
    }
    return style;
};

void add_style_screen_theme_color(lv_obj_t *obj) {
    (void)obj;
    lv_obj_add_style(obj, get_style_screen_theme_color_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};

void remove_style_screen_theme_color(lv_obj_t *obj) {
    (void)obj;
    lv_obj_remove_style(obj, get_style_screen_theme_color_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};

//
// Style: panel-imgbtn-theme
//

void init_style_panel_imgbtn_theme_MAIN_DEFAULT(lv_style_t *style) {
    lv_style_set_align(style, LV_ALIGN_CENTER);
    lv_style_set_bg_opa(style, 0);
    lv_style_set_bg_img_recolor_opa(style, 255);
    lv_style_set_bg_img_recolor(style, lv_color_hex(theme_colors[active_theme_index][0]));
    lv_style_set_shadow_color(style, lv_color_hex(theme_colors[active_theme_index][1]));
    lv_style_set_shadow_opa(style, 255);
    lv_style_set_shadow_spread(style, 0);
    lv_style_set_shadow_width(style, 0);
};

lv_style_t *get_style_panel_imgbtn_theme_MAIN_DEFAULT() {
    static lv_style_t *style;
    if (!style) {
        style = lv_mem_alloc(sizeof(lv_style_t));
        lv_style_init(style);
        init_style_panel_imgbtn_theme_MAIN_DEFAULT(style);
    }
    return style;
};

void add_style_panel_imgbtn_theme(lv_obj_t *obj) {
    (void)obj;
    lv_obj_add_style(obj, get_style_panel_imgbtn_theme_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};

void remove_style_panel_imgbtn_theme(lv_obj_t *obj) {
    (void)obj;
    lv_obj_remove_style(obj, get_style_panel_imgbtn_theme_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};

//
//
//

void add_style(lv_obj_t *obj, int32_t styleIndex) {
    typedef void (*AddStyleFunc)(lv_obj_t *obj);
    static const AddStyleFunc add_style_funcs[] = {
        add_style_container_connectivity,
        add_style_images_theme_color,
        add_style_connectivity_container_image,
        add_style_text_theme_color,
        add_style_screen_theme_color,
        add_style_panel_imgbtn_theme,
    };
    add_style_funcs[styleIndex](obj);
}

void remove_style(lv_obj_t *obj, int32_t styleIndex) {
    typedef void (*RemoveStyleFunc)(lv_obj_t *obj);
    static const RemoveStyleFunc remove_style_funcs[] = {
        remove_style_container_connectivity,
        remove_style_images_theme_color,
        remove_style_connectivity_container_image,
        remove_style_text_theme_color,
        remove_style_screen_theme_color,
        remove_style_panel_imgbtn_theme,
    };
    remove_style_funcs[styleIndex](obj);
}

