  ...
  return (
    <>
      <div className='mb-4 flex flex-row items-center justify-between gap-4'>
        <h1 className='text-2xl font-bold sm:text-3xl'>{t`Bluetooth Scales`}</h1>
        <button
          className={`btn btn-primary ${isScanning ? 'loading' : ''}`}
          onClick={onScan}
          disabled={isScanning}
        >
          {isScanning ? t`Scanning...` : t`Scan for Scales`}
        </button>
      </div>

      <div className='grid grid-cols-1 gap-4 lg:grid-cols-12'>
        <Card sm={12} title={t`Available Scales`}>
          {isLoading || isInfoLoading ? (
            <div className='flex items-center justify-center py-12'>
              <span className='loading loading-spinner loading-lg' />
              <span className='text-base-content/70 ml-3'>{t`Loading scales...`}</span>
            </div>
          ) : isError || isInfoError ? (
            <div className='alert alert-error'>
              <span>{t`Error loading scales. Please try again.`}</span>
            </div>
          ) : scaleData.length === 0 ? (
            <div className='py-12 text-center'>
              <div className='flex flex-col items-center space-y-4'>
                ...
                <div>
                  <h3 className='text-base-content text-lg font-medium'>{t`No scales found`}</h3>
                  <p className='text-base-content/70'>
                    {t`Click "Scan for Scales" to discover Bluetooth scales nearby`}
                  </p>
                </div>
              </div>
            </div>
                ...
                <div key={i} className='bg-base-200 border-base-300 rounded-lg border p-4'>
                  <div className='flex items-center justify-between'>
                    ...
                    <div className='flex items-center space-x-3'>
                      {scale.connected ? (
                        <div className='badge badge-success gap-2'>{t`Connected`}</div>
                      ) : (
                        <button
      ...
      {scaleData.length > 0 && (
        <div className='space-y-4 lg:col-span-12'>
          <div className='alert alert-info'>
            <span>
              {t`Scales are automatically refreshed every 10 seconds. Use the scan button to discover new devices.`}
            </span>
          </div>
        </div>
      )}
