...
void DefaultUI::setupReactive() {
    ...
    effect_mgr.use_effect([=] { return currentScreen == ui_BrewScreen; },
                          [=]() {
                              if (volumetricMode) {
                                  lv_label_set_text_fmt(ui_BrewScreen_targetDuration, "%.1f g", targetVolume);
                              } else {
    ...
    effect_mgr.use_effect([=] { return currentScreen == ui_GrindScreen; },
                          [=]() {
                              if (volumetricMode) {
                                  lv_label_set_text_fmt(ui_GrindScreen_targetDuration, "%.1f g", grindVolume);
                              } else {
    ...
    effect_mgr.use_effect([=] { return currentScreen == ui_BrewScreen; },
                          [=]() {
                              if (volumetricAvailable && bluetoothScales) {
                                  lv_label_set_text_fmt(ui_BrewScreen_weightLabel, "%.1f g", bluetoothWeight);
                              } else {
                                  lv_label_set_text(ui_BrewScreen_weightLabel, "-");
                              }
                          },
                          &bluetoothWeight, &volumetricAvailable, &bluetoothScales, &active);

    effect_mgr.use_effect([=] { return currentScreen == ui_BrewScreen; }, [=]() {
        // Update elapsed time on Brew screen if a brew is running (or 00:00 otherwise)
        ...
        if (process == nullptr || process->getType() != MODE_BREW) {
            lv_label_set_text(ui_BrewScreen_currentDuration, "00:00");
            return;
        }
        ...
        if (brewProcess && brewProcess->processStarted > 0 && now >= brewProcess->processStarted) {
            ...
            lv_label_set_text_fmt(ui_BrewScreen_currentDuration, "%2d:%02d", processMinutes, processSeconds);
        } else {
            lv_label_set_text_fmt(ui_BrewScreen_currentDuration, "00:00");
        }
    }, &active);
    effect_mgr.use_effect([=] { return currentScreen == ui_GrindScreen; },
                          [=]() {
                              if (volumetricAvailable && bluetoothScales) {
                                  lv_label_set_text_fmt(ui_GrindScreen_weightLabel, "%.1f g", bluetoothWeight);
                              } else {
...
void DefaultUI::updateStatusScreen() const {
    ...
    if (brewProcess && brewProcess->target == ProcessTarget::VOLUMETRIC && phase.hasVolumetricTarget()) {
        ...
        lv_label_set_text_fmt(ui_StatusScreen_brewLabel, "%.1f g", target.value);
    } else if (brewProcess) {
    ...
    if (brewProcess && brewProcess->target == ProcessTarget::TIME) {
        ...
        lv_label_set_text_fmt(ui_StatusScreen_targetDuration, "%.1f g", brewProcess->getBrewVolume());
    }
    if (brewProcess) {
    ...
    if (process->isActive()) {
        // Show a live weight readout during an active brew when a volumetric measurement source is available.
        if (volumetricAvailable && bluetoothScales) {
            lv_label_set_text_fmt(ui_StatusScreen_brewVolume, "%.1f g", bluetoothWeight);
            lv_obj_clear_flag(ui_StatusScreen_brewVolume, LV_OBJ_FLAG_HIDDEN);
        } else if (brewProcess && brewProcess->target == ProcessTarget::VOLUMETRIC) {
            lv_label_set_text_fmt(ui_StatusScreen_brewVolume, "%.1f g", brewProcess->currentVolume);
            lv_obj_clear_flag(ui_StatusScreen_brewVolume, LV_OBJ_FLAG_HIDDEN);
        } else {
